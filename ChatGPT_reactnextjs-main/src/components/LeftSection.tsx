    ...
    const fetchChats = async () => {
        try {
            ...
            if (Array.isArray(data.messages)) {
                setAllChats(data.messages); // Assuming the API returns { success: true, messages: [...] }
            } else {
                setAllChats([]); // Set to an empty array if the data is not as expected
            }
        } catch (error) {
            console.error('Error fetching chats:', error);
            setAllChats([]); // Set to an empty array in case of error
        }
    };
            ...
            <div className={styles.allChats}>
                {Array.isArray(allChats) && allChats.length > 0 ? (
                    allChats.map((chat, index) => (
                        <div key={chat._id || index} className={styles.chat}>
